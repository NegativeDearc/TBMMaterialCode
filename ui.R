...
shinyUI(pageWithSidebar( 
  ...
  sidebarPanel(width =3 ,
               ...
               actionButton("action", icon = icon('search'),label = "点击刷新")
               ),
  
  mainPanel(
    tabsetPanel(
      id = 'dataset',type = 'tabs',
      tabPanel('分类汇总', icon = icon("list-alt"),dataTableOutput(outputId = 'table1')),
      tabPanel('当日汇总表', icon = icon("list-alt"),dataTableOutput(outputId = 'table')),
      ...
      tabPanel('更新记录表',icon = icon('envelop-o'),includeMarkdown('log.md'))
      ))
)
)